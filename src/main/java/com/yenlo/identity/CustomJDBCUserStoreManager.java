...
//		}
     ...
     * @throws UserStoreException
     */
    public CustomJDBCUserStoreManager(DataSource ds, RealmConfiguration realmConfig)
            throws UserStoreException {
    ...
    public CustomJDBCUserStoreManager(RealmConfiguration realmConfig, Map<String, Object> properties,
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("The jdbcDataSource being used by CustomJDBCUserStoreManager :: "
                        + jdbcds.hashCode());
            }
        } catch (Exception e) {
            if (log.isDebugEnabled()) {
...
//
     ...
     * @throws UserStoreException from the getUserNames()
    ...
    public void doAddUser(String userName, Object credential, String[] roleList,
        ...
        log.info(" customJDBCUserStoremanger________________________________ adding user");
        try {
            Connection connection = getDBConnection();
            PasswordHistoryVerfication passwordHistoryVerfication = new PasswordHistoryVerfication(connection);
            ...
            password = this.preparePassword(password, saltValue);
            addNewUser = passwordHistoryVerfication.addnewUser(connection, userName, password);
            log.info(" customJDBCUserStoremanger__ user added in passwordHistoryTable  " +addNewUser);
        ...
        if (addNewUser) {
            log.info(" customJDBCUserStoremanger__ calling persist user " + addNewUser);
    ...
    public void doUpdateCredentialByAdmin(String userName, Object newCredential)
        ...
        log.info(" customJDBCUserStoremanger________________________________ changing  password");
        boolean passwordConditionVerified = false;
        // custom changes made; check if the password change requirement has met
        try {
            Connection connection = getDBConnection();
            PasswordHistoryVerfication passwordHistoryVerfication = new PasswordHistoryVerfication(connection);

            String saltValue = null;
            String password = (String) newCredential;
            ...
            password = this.preparePassword(password, saltValue);
            passwordConditionVerified = passwordHistoryVerfication.updatePassword(connection, userName, password);
            log.info("customJDBCUserStoremanger__ updated user password in passwordHistoryTable  "+passwordConditionVerified);
            //DatabaseUtil.closeConnection(connection);
        } catch (UserStoreException e) {
            e.getMessage();
            throw new UserStoreException(e.getMessage(), e);
